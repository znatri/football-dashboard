...
class ViewTransformer:
    def __init__(self,):
        ...
        if self.pixel_vertices.shape[0] != self.target_vertices.shape[0]:
            raise ValueError("Number of pixel vertices must match number of target vertices.")
    ...
    def transform_point(self, point: List[float]) -> Optional[np.ndarray]:
        ...
        is_inside = cv2.pointPolygonTest(self.pixel_vertices, (p[0][0], p[0][1]), False) >= 0
        if not is_inside:
            return None
        
        transformed_point = cv2.perspectiveTransform(p.reshape(-1, 1, 2), self.perspective_transform)
