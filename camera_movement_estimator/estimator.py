...
class CamerMovementEstimator:
        ...
        for frame_num, frame in enumerate(frames[1:], start=1):
            ...
            if new_features is not None and old_features is not None:
                ...
                for i, (new, old) in enumerate(zip(new_features, old_features)):
                    ...
                    if dist > max_dist:
                        max_dist = dist
                        camera_movement_x, camera_movement_y = measure_xy_distance(old, new)
    ...
    def draw_camera_movement(self, frames: List[np.ndarray], camera_movement_per_frame: List) -> List[np.ndarray]:
        ...
        for frame_num, frame in enumerate(frames):
            ...
            cv2.putText(annotated_frame, f"Camera Movement X: {x_movement:.2f}", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 0), 3)
            cv2.putText(annotated_frame, f"Camera Movement Y: {y_movement:.2f}", (10, 60), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 0), 3)
