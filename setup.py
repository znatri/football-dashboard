...
def create_virtualenv():
    try:
        subprocess.run(['virtualenv', '-p', '$(which python3.10)', '--system-site-packages', 'football_env'], check=True, shell=True)
        print("Virtual environment created successfully.")
    except subprocess.CalledProcessError:
        print("Failed to create virtual environment. Please ensure virtualenv is installed.")

def install_requirements():
    try:
        subprocess.run(['./football_env/bin/pip', 'install', '-r', 'requirements.txt'], check=True, shell=True)
